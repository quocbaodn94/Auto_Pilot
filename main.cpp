...
int main()
{
   ...
   while(cap.isOpened()){
       ...
       int angle = atan2(I.x-j/2, I.y) * 180 / CV_PI;
       ostringstream lable;
       lable << angle;
       ...
       resizeWindow("Display window", 400, 300);
       ...
       if(abs(angle - old_angle) > 5 && angle < 45 && angle > -45){
           char tt[10];
           sprintf(tt, "%d", angle);
           serialPuts(fd, tt);
           serialPutchar(fd, '.');
       ...
       if((char)27 == waitKey(1)){// Wait for a keystroke in the window
           serialPutchar(fd, '*');
           serialFlush(fd);
           while(1){
